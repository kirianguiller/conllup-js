...
test('returnTokensInOrder', () => {
    expect((0, conll_1.returnTokensInOrder)(treeJsonReplacedArrayWithGroup).map((token) => {
        return token.ID;
    })).toStrictEqual(['1', '2', '3', '4', '5', '6', '7']);
    expect((0, conll_1.returnTokensInOrder)(treeJsonReplacedArrayWithGroup, true).map((token) => {
        return token.ID;
    })).toStrictEqual(['1', '2', '2.2', '3', '4', '5', '6', '6.1', '7']);
    expect((0, conll_1.returnTokensInOrder)(treeJsonReplacedArrayWithGroup, true, true).map((token) => {
        return token.ID;
    })).toStrictEqual(['1-2', '1', '2', '2.2', '3', '4', '5', '6-7', '6', '6.1', '7']);
});
describe('getNodeFromTreeJson', () => {
    const mockNodeJson = (0, conll_1.emptyTokenJson)();
    mockNodeJson.ID = '1.2';
    const mockGroupJson = (0, conll_1.emptyTokenJson)();
    mockGroupJson.ID = '1-2';
    const mockNormalJson = (0, conll_1.emptyTokenJson)();
    mockNormalJson.ID = '1';
    const mockTreeJson = {
    ...
    it('should return the correct enhanced node', () => {
        expect((0, conll_1.getNodeFromTreeJson)(mockTreeJson, '1.2')).toEqual(mockNodeJson);
    });
    it('should return the correct group node', () => {
        expect((0, conll_1.getNodeFromTreeJson)(mockTreeJson, '1-2')).toEqual(mockGroupJson);
    });
    it('should return the correct normal node', () => {
        expect((0, conll_1.getNodeFromTreeJson)(mockTreeJson, '1')).toEqual(mockNormalJson);
    });
    it('should throw an error if the node does not exist and throwErrorIfMissing is true', () => {
        expect(() => (0, conll_1.getNodeFromTreeJson)(mockTreeJson, 'non-existent', true)).toThrowError('node non-existent not found in treeJson and throwErrorIfMissing is true');
    });
    it('should return undefined if the node does not exist and throwErrorIfMissing is false', () => {
        expect((0, conll_1.getNodeFromTreeJson)(mockTreeJson, 'non-existent', false)).toBeNull();
    });
    it('should return undefined if the node does not exist and throwErrorIfMissing is not provided', () => {
        expect((0, conll_1.getNodeFromTreeJson)(mockTreeJson, 'non-existent')).toBeNull();
    });
});
